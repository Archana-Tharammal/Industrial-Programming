...
namespace Web_server
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void add_to_fav_button_Click_1(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(url))
            {
                ...
                if (!string.IsNullOrWhiteSpace(name))
                {
                    ...
                    favourite_listbox.Items.Add(favItem);       //Add to favourites listbox

                    try
                    {
                        //Append favourite to file
                        System.IO.File.AppendAllLines(favouritesFile, new[] { favItem });
                    ...
                    {
                        MessageBox.Show($"An error occurrs while saving to favourites: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
                else
                {
        ...
        //Loads saved favourites from file into the favourites listbox
        private void Load_Favourites()
        {
            favourite_listbox.Items.Clear();
            if (System.IO.File.Exists(favouritesFile))
            {
                var favourites = System.IO.File.ReadAllLines(favouritesFile);
                foreach (var favourite in favourites)
                {
                    favourite_listbox.Items.Add(favourite);     //Load favourites into listbox
                }
            }
        ...
        private void Edit_fav_button_Click(object sender, EventArgs e)
        {
            if (favourite_listbox.SelectedItem is string selectedFav)
            {
                ...
                //Modifying options for name and URL  
                ...
                try
                {
                    ...
                    favourite_listbox.Items.Remove(selectedFav);        //Remove old favourite
                    string updatedFav = $"{newName}-{newUrl}";
                    favourite_listbox.Items.Add(updatedFav);        //Add updated favouite to list
                    Save_Favourites();                              //Save changes to file
        ...
        private void html_Textbox_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                string pagetitle = htmlContent.Substring(titleStart + 7, titleEnd - (titleStart + 7)).Trim();
